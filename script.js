...
//função de pesquisar o nome do produto e achar o produto somente 

/*const pesquisar=document.getElementById('pesquisar')

pesquisar.addEventListener('click',function(){
    let digiteaqui=document.getElementById('Buscar').value
    console.log(digiteaqui)
    for(let i=0;i<dados.length;i++){
        let nomesdiferentes=digiteaqui!==dados[i].nome
        let produtosvitrine= document.getElementsByClassName('produtos')
        let itenstirar= produtosvitrine[i]===dados[i]
        
        console.log(itenstirar)
        if(digiteaqui===dados[i].nome){
         vitrine.removeChild(produtosvitrine[i])
        }
    }
})*/
